...
const registerBusiness = async (req, res) => {
  ...
  const { name, description,
     website, country,
      openHour, closeHour,
       category,hasPhysicalAddress, regNumber, teamSize} = req.body
  // const ownerId = req.user.userId
  const business = { name, hasPhysicalAddress,
     regNumber, owner: userId,
      address: busOwner.address, description, website, country,
       openHour, closeHour, category, teamSize }
  const newBusiness = await Business.create({ ...business})

  busOwner = await User.updateOne({_id: userId}, {$set:{business: newBusiness._id}})
...
const inviteStaff = async (req, res) => {
 ...
 const url = `http://localhost:4000/api/v1/auth/staff-verify/${String(token)}`
 await staffOnboard(staffEmail, name, url, owner.firstName + " " + owner.lastName, owner.email)
...
const staffRegister = async (req, res) => {
  const { staffEmail, role, business } = jwt.verify(req.params.token, process.env.JWT_SECRET)
  const { firstName, lastName, phoneNumber, address, password} = req.body
  
  const user = { firstName, lastName, email: staffEmail, phoneNumber, address, business, role, password, status: "active" }
  const staff = await User.create({ ...user})
  const token = staff.createJWT()
  res.status(StatusCodes.CREATED).json({ staff: {FirstName: staff.firstName, LastName: staff.lastName, staff: user.email, business: staff.business}, token })
}
...
const login = async (req, res) => {
   ...
   if(user.status == "pending") throw new UnauthenticatedError('You need to verify your account')

   const isPasswordCorrect = await user.comparePassword(password)
   if(!isPasswordCorrect) throw new UnauthenticatedError('Invalid credentials, wrong password')
